...
from config import BASE_MODEL, SFT_ADAPTER, DPO_ADAPTER, DPO_FILTERED_ADAPTER
...
model = PeftModel.from_pretrained(model, SFT_ADAPTER)
model = model.merge_and_unload()
model = PeftModel.from_pretrained(model, DPO_FILTERED_ADAPTER)
for p in prompts:
    print(f"\nPrompt: {p}")
    print(f"Response: {generate(model, tokenizer, p)}")
    print("-" * 40)
